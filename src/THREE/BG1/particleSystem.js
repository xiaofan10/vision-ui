...
const BuiltinShaderAttributeName = [
  "position",
  "normal",
  "uv",
  "color",
...
const { VerticesDuplicateRemove } = utils;
...
class ParticleSystem {
  ...
  createScene() {
    ...
    this.scene.background = new THREE.Color(0x0c1f52);
    // 创建相机
  ...
  destory() {
    ...
    this.scene.children.forEach((item) => {
      if (item.dispose) {
        ...
        item.geometry?.destory?.();
        item.material?.destory?.();
      }
    });
    this.scene = null;
    this.AnimateEffectParticle = null;
    this.addons = null;
    // 清除渲染器
    ...
    this.modelList = null;
    // 清除动画组
    if (this.MainParticleGroup != null) {
      this.MainParticleGroup.removeAll();
    }
  }
}
